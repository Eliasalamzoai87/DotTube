...
<?php
    ...
    if(!$__user_h->user_exists($_GET['n']))
        header("Location: /?error=This user doesn't exist");
	...
	while($ban = $stmt->fetch(PDO::FETCH_ASSOC)) { 
		header("Location: /?error=This user has been terminated for violating DotTube's Community Guidelines.");
	}
...
?>
<?php
	$__server->page_embeds->page_title = "SubRock - " . htmlspecialchars($_user['username']);
